...
export class Mapping {
	...
	constructor(rawData: MappingData) {
		...
		this._commandDescription = getCommandDescriptionOrUnknown(
			rawData.traktorControlId,
		);
	...
	copy(includeMidiBinding = false): Mapping {
		...
		const copiedSettings: MappingSettingsData = {
			...
			conditionOneValue: new Uint8Array(
				this._rawData.settings.conditionOneValue,
			),
			conditionTwoValue: new Uint8Array(
				this._rawData.settings.conditionTwoValue,
			),
			ledMinControllerRange: new Uint8Array(
				this._rawData.settings.ledMinControllerRange,
			),
			ledMaxControllerRange: new Uint8Array(
				this._rawData.settings.ledMaxControllerRange,
			),
		};

		const copiedData: MappingData = {
			midiNoteBindingId: includeMidiBinding
				? this._rawData.midiNoteBindingId
				: -1,
 ...
 * Format: "CC.XX.YYY" or "Note.XX.YYY" where XX=channel, YYY=note/cc number
 */
export function parseMidiNoteString(noteString: string): MidiBinding | null {
	...
	const isCC = typeStr.toLowerCase() === "cc";
