 ...
 *
 * Phase 19: Added folder/file picker functionality for paths settings.
 */
...
import { type AppSettings, useAppStore, useSettings } from '../../store/appStore';
...
type SettingsTab = 'paths' | 'traktor' | 'tsi' | 'editor' | 'confirmations' | 'midi' | 'advanced';
      ...
      >
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex-1 p-6 overflow-auto">
            {activeTab === 'paths' && (
              <PathsSettings settings={settings.paths} onUpdate={updatePathsSettings} />
            )}
            {activeTab === 'traktor' && (
              <TraktorSettingsPanel settings={settings.traktor} onUpdate={updateTraktorSettings} />
            )}
            {activeTab === 'tsi' && (
            ...
            {activeTab === 'editor' && (
              <EditorSettings settings={settings.editor} onUpdate={updateEditorSettings} />
            )}
            ...
            {activeTab === 'midi' && (
              <MidiSettings settings={settings.midi} onUpdate={updateMidiSettings} />
            )}
            {activeTab === 'advanced' && (
              <AdvancedSettings settings={settings.advanced} onUpdate={updateAdvancedSettings} />
            )}
          </div>
        </div>
  ...
  const handleBrowse = async () => {
    try {
      ...
      if (browseType === 'folder') {
        selectedPath = await ipcClient.selectFolder(value || undefined, `Select ${label}`);
      } else {
        // For TSI files
        selectedPath = await ipcClient.selectFile(value || undefined, `Select ${label}`, [
      ...
      <div>
        ...
        <div className="space-y-5">
          ...
          <PathInput
            ...
            onChange={(value) => onUpdate({ pathToControllerDefaultMappings: value })}
...
function TraktorSettingsPanel({ settings, onUpdate }: TraktorSettingsPanelProps) {
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-sm font-medium mb-4">Traktor Version</h3>
        <p className="text-xs text-muted-foreground mb-4">
          This version is written into TSI files, so Traktor can check compatibility.
        </p>
        <div className="space-y-4">
          <div className="space-y-1.5">
            ...
            <Input
              ...
              className={cn(!settings.overrideTraktorVersion && 'opacity-50 italic')}
            />
          </div>
          <Checkbox
            ...
            onChange={(e) => onUpdate({ overrideTraktorVersion: e.target.checked })}
          />
        </div>
      </div>
    </div>
...
function TsiOptimizationSettings({ settings, onUpdate }: TsiOptimizationSettingsProps) {
  return (
    <div className="space-y-6">
      <div>
        ...
        <div className="space-y-4">
          <Checkbox
            ...
            onChange={(e) => onUpdate({ removeUnusedMidiDefinitions: e.target.checked })}
          />
          <Checkbox
      ...
      <div>
        ...
        <div className="space-y-4">
          ...
          <Checkbox
            ...
            onChange={(e) => onUpdate({ showConditionsColumn: e.target.checked })}
          />
          <Checkbox
      ...
      <div>
        ...
        <div className="space-y-4">
          <Checkbox
            ...
            onChange={(e) => onUpdate({ clearFilterAtPageChanges: e.target.checked })}
          />
          <Checkbox
            ...
            onChange={(e) => onUpdate({ clearFilterAtModifications: e.target.checked })}
          />
          <div className="space-y-1.5">
            ...
            <Input
              ...
              onChange={(e) => onUpdate({ filterMenuSize: Number(e.target.value) || 20 })}
              className="w-24"
            />
          </div>
        </div>
...
function ConfirmationsSettings({ settings, onUpdate }: ConfirmationsSettingsProps) {
  return (
    <div className="space-y-6">
      <div>
        ...
        <div className="space-y-4">
          <Checkbox
            ...
            onChange={(e) => onUpdate({ confirmDeleteDevices: e.target.checked })}
          />
          <div className="space-y-1.5">
            <label htmlFor="delete-mappings-threshold" className="text-sm font-medium">
              Mapping delete confirmation threshold
            </label>
            <p className="text-xs text-muted-foreground">
              Show confirmation when deleting this many mappings or more (0 = disabled)
            </p>
            <Input
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="flex items-center gap-4">
          <label htmlFor="learn-timeout" className="text-sm text-muted-foreground">
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) => onUpdate({ defaultOutputDevice: e.target.value })}
              placeholder="(Auto-detect)"
            />
          </div>
        </div>
      ...
      <div>
        ...
        <div className="space-y-4">
          <Checkbox
            ...
            onChange={(e) => onUpdate({ loadLastFileAtStartup: e.target.checked })}
          />
        </div>
      </div>
