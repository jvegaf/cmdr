...
function DeviceListItem({
  ...
  return (
    <div className="relative group">
      {/* Main row container - onContextMenu is for right-click menu, not primary interaction */}
      {/* biome-ignore lint/a11y/noStaticElementInteractions: onContextMenu is auxiliary, not primary interaction */}
      <div
        className={cn(
      ...
      >
        ...
        {/* Device info - clickable button to select */}
        <button
          type="button"
          className="flex-1 truncate text-left bg-transparent border-0 p-0 cursor-pointer"
          onClick={onSelect}
        >
      ...
      {showMenu && (
        <>
          {/* Backdrop to close menu - using button for accessibility */}
          <button
            type="button"
  ...
  return (
    <div className="space-y-1">
      {devices.map((device, index) => (
        <DeviceListItem
          // AIDEV-NOTE: Use index as fallback key if device.id is -1 (multiple devices can have id=-1)
          key={device.id >= 0 ? `device-${device.id}` : `device-index-${index}`}
          device={device}
