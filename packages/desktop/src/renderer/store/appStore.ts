...
export function useColumnWidths(): ColumnWidths {
	return useAppStore(
		(state) => state.settings?.editor?.columnWidths ?? DEFAULT_COLUMN_WIDTHS,
	);
}
 ...
 */
export function useUpdateColumnWidths(): (
	updates: Partial<ColumnWidths>,
) => void {
