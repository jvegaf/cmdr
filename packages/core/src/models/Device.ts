...
export class Device {
	...
	static create(
		...
		return new Device(
			createDefaultDevice(deviceType, traktorVersion, isKeyboard),
		);
	}
	...
	incrementRevision(): void {
		this._rawData.data.version.mappingFileRevision =
			(this._rawData.data.version.mappingFileRevision + 1) % 2147483647;
	}
	...
	getMidiDefinition(
		note: string,
		isOutput: boolean,
	): MidiDefinitionData | undefined {
		return isOutput
			? this._midiOutDefinitions.get(note)
			: this._midiInDefinitions.get(note);
	}
		...
		if (this._rawData.data.mappings) {
			...
			if (!keepBinding && mapping.midiNoteBindingId >= 0) {
				...
				const bindingIndex = bindings.findIndex(
					(b) => b.bindingId === mapping.midiNoteBindingId,
				);
				if (bindingIndex >= 0) {
	...
	private _createNewBindingId(): number {
		...
		if (maxId < 2147483646) return maxId + 1;
				...
				mappings: data.mappings.mappings.map((m) => ({
					...
					settings: {
						...
						ledMinControllerRange: new Uint8Array(
							m.settings.ledMinControllerRange,
						),
						ledMaxControllerRange: new Uint8Array(
							m.settings.ledMaxControllerRange,
						),
					},
				})),
